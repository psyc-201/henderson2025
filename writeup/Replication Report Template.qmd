...
## Project Progress Check 2 (Reproducability)
...
Additionally, as a visual check, I will plot the overall decoding accuracy patterns shown in Figure 2A–C to confirm that my pattern of results is simlar to the original paper. To make this comparison a bit smoother, I also imported their plotting helpers so that I can check my ability reproduce their exact Figure 2A-C. I expect that the means may shift, but hope the general pattern will hold. To reiterate, I anticipate higher accuracy for the Linear tasks than the Nonlinear task, with a similar pattern of accuracies (and significance) across ROIs.

## Project Progress Check 3 (Continuing Reproducability + Beginning Simulation)
As an update from the last progress check, I tried to implement the authors' conversion of the mat files to the h5py format from scratch, but it was still a bit of a headache/ordeal. However, it is in progress in that I have the Matlab script that compiles the data in a Python-friendly way successfully, but I haven't finished editing my decoding script to replace my calls to the authors' helper functions, so I've still been using the version that has the authors' helpers just to keep forward momentum. I hope to revisit this before the final project deadline to see if I can truly re-implement the whole pipeline from scratch, but I feel like it's less of a priority/a less valuable experience for me.

I finished training the binary decoders, and judging by Figures 2A-C (attached below), the pattern of data in my reproducability attempt generally match theirs:

I also ran the 3-way ANOVA (ROI × Task × Boundary) on the classification accuracy from the binary decoder above (includes data for near-trials only, labels: Linear1 vs. Linear2). Like the authors, I detected a significant interaction between classifier boundary and task, such that the classifier performed better when the boundary matched th one that was currently activity in the task (p=.0X). This effect was of a _____ effect size, which is similar to the ___ effect size I calculated based off the results reported by the authors. In sum, I would say my reproducability attempt was successful.

I also began the simulation portion of my experience (creating a new batch of stimuli). Specifically, I created the 36 images that are a part of the 6x6 array I described above (see image below, where I arranged them all in the grid). The authors had about a dozen different scripts in Matlab that they used for stimulus generation. I re-purposed/implemented the legacy functions they used (giivng proper credit), and basically re-consolidated all the functionality to one script. I varied the amplitude of 2 RFCs different from the ones the authors used. I also chose to use a different "shape family." My next step will be running the images through the computer vision models.



-----
As a side note, I've doing most of my coding (especially for the classification) within .py files in the Spyder IDE. I do have a .qmd version of the classification pipeline (understand this is more in line with the class objectives, especially because Quarto/Pixi lend themselves to reproducability), but it's been MUCH easier for me to crash-test my code in Spyder's IDE. Moreover, I ran the models on one of the external neurocomputing servers in a .py file because it was easiest for me. Once everything is finalized, I'll update/shift any scripts over to be .qmd.
