...
In the original study, the authors trained logistic regression classifiers to predict category membership for the Linear-1, Linear-2, and Nonlinear boundaries using voxel activation patterns from several visual ROIs. Henderson et al. (2025) reported that classifier accuracy depended on the current task (i.e., which boundary participants were using at that moment). A three-way ANOVA including ROI, Task, and Boundary showed that when participants performed the Linear-2 task, the Linear-2 classifier was better at predicting a shape’s category than the Linear-1 classifier. This effect was specific to shapes near the decision boundary (Task × Boundary: *F*(1,9) = 8.99, *p* = .011, ηₚ² = 0.50). The authors interpreted this interaction as evidence that the structure of category representations (i.e., which boundary best separates neural patterns) shifts depending on the task context.

I aimed to reproduce this Task × Boundary interaction. I also attempted to reproduce the broad pattern of classifier accuracies reported in the original paper: decoding accuracies should be highest in early visual ROIs (V1, V2, V3) and lowest in later visual ROIs (e.g., IPS), and accuracy for the Nonlinear classifier should be lower than for the Linear-1 and Linear-2 classifiers.

**Simulation**  
Separate from this reproducibility test, I created a new sample of novel shape stimuli that varied continuously across two features and examined whether they yielded the same key properties of the original set. Particularly, I checked that the new stimuli roughly conformed to a two-dimensional shape space after dimensionality reduction, and whether, perceptually, stimuli were most separable across the Linear-2 and Linear-1 categories compared to the Nonlinear category, as in Henderson et al. (2025).
