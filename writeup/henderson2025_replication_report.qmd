...
I finished training the binary decoders, and judging by Figures 2A-C (attached below), the pattern of data in my reproducibility attempt generally match theirs:

I also ran the 3-way ANOVA (ROI × Task × Boundary) on the classification accuracy from the binary decoder above (includes data for near-trials only, labels: Linear1 vs. Linear2). Like the authors, I detected a significant interaction between classifier boundary and task, such that the classifier performed better when the boundary matched the one that was currently activity in the task (p=.0X). This effect was of a _____ effect size, which is similar to the ___ effect size I calculated based off the results reported by the authors. In sum, I would say my reproducibility attempt was successful.
...
![6×6 RFC blob grid.](../stimuli/new_blob_stim/grid_montage.png){#fig-blobgrid fig-cap="Blob grid (6×6) spanning 0–5 RFC amplitudes on each axis." fig-alt="36 grayscale blob silhouettes arranged in a 6×6 grid." width="75%" fig-align="center"}

**Link to Preregistration** [Google Drive - ](https://github.com/Lena-Kemmelmeier/henderson2025)
