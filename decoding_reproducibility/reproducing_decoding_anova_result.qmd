...
Run thic cell below to initialize the helper functions I have created
```{python}
...
Run this cell below to decode the voxel activation data. This is specifically to recreate Figure 2 so there is no no near/far split of the data.
```{python}
...
Run this cell below to decode the data (this uses near only data: labels are linear1 vs. linear2)
```{python}
...
Run this cell below to run a 3-way ANOVA (ROI × Task × Boundary) on the data from above
```{python}
aov = run_task_boundary_roi_anova(acc_long_near, out_csv=os.path.join(stats_output_path, 'anova_table_NEARONLY.csv'))
```

Run this cell to recreate Figure 2A–C (I copied the authors' style here -- using their exact figure code just to make a direct comparison)
```{python}
